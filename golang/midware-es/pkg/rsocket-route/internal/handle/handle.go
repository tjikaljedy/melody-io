...
import (
	"errors"
	"fmt"
	"melody-io/midware-es/pkg/rsocket-route/decode"
...
func Paths(paths map[string]interface{}) (err error) {
	for path, genericMethod := range paths {
		...
		case func(msgs rx.Publisher) flux.Flux:
			...
			rc[path] = method
		default:
			err = fmt.Errorf(fmt.Sprintf("%v for %s is unknown", genericMethod, path))
		}
	}
	return
}
...
func pathError(path string) error {
	return fmt.Errorf(fmt.Sprintf("path: %s duplicated", path))
}
...
func requestChannel() rsocket.OptAbstractSocket {
	return rsocket.RequestChannel(func(msgs flux.Flux) flux.Flux {
		return msgs.SwitchOnFirst(func(sig flux.Signal, f flux.Flux) flux.Flux {
